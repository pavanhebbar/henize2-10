...
def fit_funcs(xdata, ydata):
    ...
    lboun = [0., binsize*2, np.min(xdata), 0.]
    uboun = [np.max(ydata), 2.*xlen, np.max(xdata), np.max(ydata)]
    test_periods = xlen*1.0/np.arange(1.0, 1.0*int(len(xdata)/2.+0.5), 1.)
    for period in test_periods:
        sin_val, sin_e = curve_fit(func_sine, xdata, ydata,
                                   p0=[0., period, np.min(xdata),
                                       np.mean(ydata)], bounds=(lboun, uboun),
                                   sigma=1+(ydata+0.75)**0.5)
...
def monte_carlo_step(opt=1, optparam=[]):
    ...
    if opt == 1:
        ...
        xvals = optparam[1].copy()
        yvals = np.random.poisson(optparam[2], len(xvals))
    else:
...
def monte_carlo(ntimes, opt=1, optparam=[]):
    ...
    for i in range(ntimes):
        func_fits = monte_carlo_step(opt, optparam)
...
def plotfig(name, xlabel, ylabel, xdatas, ydatas, legends=[]):
    ...
    for i, xdata in enumerate(xdatas):
        plt.plot(xdata, ydatas[i], label=legends[i])
...
def get_stats(const_params, sin_params):
    """Return information on the constant the sine fit params
    ...
    plotfig('chi_vals.png', r'$\chi$', r'$f$', [chi_vals, chi_vals],
    ...
    plotfig('aic_vals.png', 'AIC value', r'$f$', [aic_vals, aic_vals],
    ...
    plotfig('amp_sin.png', 'Amplitude', r'$f$', [conf_vals],
...
def main(args):
    ...
    const_params, sin_params = monte_carlo(
        ntimes, opt, [xlim[0], xlim[1], length, binsize])
    np.savetxt('const_params_'+str(opt)+'.txt', const_params)
    np.savetxt('sin_params_'+str(opt)+'.txt', sin_params)
